...
FORMAT_ASS="\033[95mAssociation\033[00m ... %-2d made by %-20s "
...
while mgr.next_event():
    ...
    for x in xrange(product_id_v.size()):
        ...
        if product_ptr :
            try:
                if not product_id.first == fmwk.data.kAssociation:
                    print FORMAT_VEC % (mgr.product_name(product_id.first),product_id.first,product_id.second,product_ptr.size())
                else:
                    print FORMAT_ASS % (product_ptr.size(),product_id.second)
                    product_ptr.list_association()
